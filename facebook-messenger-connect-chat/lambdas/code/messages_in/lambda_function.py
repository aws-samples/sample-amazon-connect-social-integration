## process Facebook Messenger webhook messages
import json
import os
import logging
...
def get_contact(table_service, user_id):
    """Get contact from DynamoDB by user_id using the byUser GSI"""
...
def text_message_handler(message, connect_chat_service, table_service, user_name, sender_profile):
    ...
    Args:
        ...
        sender_profile: Messenger profile data for the sender
...
def download_attachment(url):
    """Download attachment content from Messenger CDN URL.

    Args:
        url: The Messenger CDN URL for the attachment
...
def get_attachment_filename(attachment):
    """Generate a filename based on attachment type and MIME type.
    ...
    Args:
        attachment: Messenger attachment dict with 'type' and 'payload'
    ...
    return f"messenger_{att_type}.{ext}"
...
def attachment_message_handler(message, connect_chat_service, table_service, user_name, sender_profile):
    ...
    Args:
        message: MessengerMessage with attachments
        connect_chat_service: ChatService instance
        table_service: TableService instance
        user_name: Display name for the user
        sender_profile: Messenger profile data
...
def lambda_handler(event, context):
    ...
    try:
        ...
        if event['httpMethod'] == 'GET':
            return build_response(200, validate_healthcheck(event, config['MESSENGER_VERIFICATION_TOKEN']))
        
        # Handle POST request (webhook message)
        # Handle null/empty body → return HTTP 200
        ...
        # Get Messenger Page access token
        access_token = get_secret_value(os.environ["SECRET_ARN"])
        
        # Initialize Messenger service with get_profiles=True to automatically fetch profiles
        # MessengerService only processes webhooks with object: "page"
        messenger = MessengerService(body, page_id=page_id, access_token=access_token, get_profiles=True)
        
        logger.info(f"Parsed {messenger.get_message_count()} messages")
        ...
        for message in messenger.get_messages():
            ...
            sender_profile = messenger.user_profiles.get(message.sender_id)
            if sender_profile:
                logger.info(f"Sender profile: {sender_profile.get('first_name')} {sender_profile.get('last_name')}")
                # Use get_display_name to concatenate first_name + last_name
                user_name = messenger.get_display_name(sender_profile, message.sender_id)
            ...
            # Store pageId instead of instagramAccountId
            if new_contact_id:
