...
To remove a deployed stack and all its associated resources from AWS:

```bash
cdk destroy
```

To destroy a specific stack (if your app defines multiple):
```bash
...
- Some resources (like S3 buckets with objects or DynamoDB tables) may have deletion protection or retention policies that prevent automatic removal. You may need to empty or manually delete those resources first.
- Destroying a stack is irreversible. Any data stored in resources without backups will be lost.
- The bootstrap stack (`CDKToolkit`) is not removed by `cdk destroy`. Remove it separately via the CloudFormation console if needed.
