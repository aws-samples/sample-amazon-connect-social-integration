...
4. The sender's Instagram profile is retrieved via the Graph API — including, among others,  `name`, `username`, `profile_pic`, `follower_count`. The code stores these profiles in a user database (e.g., a DynamoDB instagram profiles table) and check it before calling the Graph API; only re-fetch when the cached profile is stale or missing, reducing API calls and latency on repeat conversations.
...
| `senderProfile` | sender instagram public profile |
...
_Sending Documents from instagram user app looks like is not possible, but users can receive documents from Amazon Connect ATTACHMENTS_

## Deployment

### Prerequisites

To receive Instagram DMs from meta you will need (not covered with this deployment):

- An Instagram Business or Creator account configured with Meta's Instagram API. For detailed instructions follow the [Instagram Setup Guide](../instagram_setup.md).
- An Amazon Connect Instance (**INSTANCE_ID**) and a Chat Inbound Contact Flow (**CONTACT_FLOW_ID**). For detailed instructions follow the [Amazon Connect Prerequisites](../general_connect.md).
...
2. Set the **Callback URL** to the API Gateway URL. You can find it in the SSM parameter `/meta/webhook/url`.
...
5. Try sending images in both directions and files from amazon connect agent.
