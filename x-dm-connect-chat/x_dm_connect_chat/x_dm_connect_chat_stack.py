...
class XDmConnectChatStack(Stack):
    ...
    def create_resources(self):
        self.secrets = secretsmanager.Secret(
            ...
            secret_string_value=SecretValue(json.dumps({
    ...
    def set_up_env_vars(self):
        for l in [
            ...
            l.add_environment("CONFIG_PARAM_NAME", config.X_CONFIG_PARAM_NAME)
        self.lambda_functions.messages_in.add_environment(
            "TOPIC_ARN", self.topic_messages_out.topic.topic_arn
        )
        self.lambda_functions.messages_in.add_environment(
            "USERS_TABLE_NAME", self.tables.x_users.table_name
        )

    def create_parameters(self):
        self.config_parameter = self.create_ssm_parameter(
            config.X_CONFIG_PARAM_NAME,
            json.dumps(config.X_CONFIG_PARAM_CONTENT),
        )
        self.create_ssm_parameter(
            config.X_WEBHOOK_PARAM_NAME,
            self.webhook.api.url_for_path("/webhooks"),
        )
    ...
    def set_up_permissions(self):
        ...
        self.tables.x_users.grant_read_write_data(
            self.lambda_functions.messages_in
        )
        ...
        self.config_parameter.grant_read(self.lambda_functions.messages_in)
